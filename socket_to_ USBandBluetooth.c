...
#define BUFSIZE2 16432
void *sendtest()
{
    char sendBuf[BUFSIZE2] = {'\0'};
    memset(sendBuf, 0, BUFSIZE2);
    while (1)
    {
        if (write(bt_fd, sendBuf, BUFSIZE2) < 0)
        {
            perror("write");
            exit(-1);
        }
    ...
    while (NotRecHeartNum != 0)
    {
        ...
        if (recvheartflag == 1) // 如果是蓝牙断开状态的话，才去连接
        {
            ...
            if (c_fd[i] > 0) // bt_fd
            {
                ...
                // pthread_t send_tid;
                // pthread_create(&send_tid, NULL, sendtest, (c_fd + i));
